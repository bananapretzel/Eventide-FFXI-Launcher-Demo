...
export async function checkForResumableDownload(): Promise<DownloadProgress | null> {
  ...
  if (actualSize !== progress.bytesDownloaded) {
    log.warn(
      chalk.yellow(
        `[checkForResumableDownload] File size mismatch: stored ${progress.bytesDownloaded}, actual ${actualSize}. Using actual size.`,
      ),
    );
  ...
  log.info(
    chalk.cyan(
      `[checkForResumableDownload] Found resumable download: ${progress.bytesDownloaded} / ${progress.totalBytes} bytes (${Math.round((progress.bytesDownloaded / progress.totalBytes) * 100)}%)`,
    ),
  );
  return progress;
}
...
export async function cancelDownload(destPath?: string): Promise<void> {
  ...
  if (pathToDelete) {
    try {
      ...
      log.info(
        chalk.cyan(
          `[cancelDownload] Deleted partial download: ${pathToDelete}`,
        ),
      );
    } catch (err) {
      log.warn(
        chalk.yellow(
          `[cancelDownload] Could not delete partial download:`,
          err,
        ),
      );
    }
