...
describe('Network Module', () => {
  ...
  describe('fetchJson', () => {
    it('should fetch and parse valid JSON', async () => {
      const result = await fetchJson('https://api.github.com/repos/microsoft/vscode/releases/latest');
  ...
  describe('downloadFile', () => {
    it('should download a small file successfully', async () => {
      ...
      const url = 'https://raw.githubusercontent.com/microsoft/vscode/main/package.json';
    ...
    it('should verify file size when expectedSize is provided', async () => {
      ...
      await expect(
        downloadFile(url, dest, undefined, 0, 0, 999999999)
    ...
    it('should throw error for non-200 status', async () => {
      ...
      await expect(
        downloadFile('https://raw.githubusercontent.com/nonexistent/nonexistent/main/file.txt', dest)
      ).rejects.toThrow();
    }, 30000);
  });
