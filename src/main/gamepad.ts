 ...
 */
const REGISTRY_PATH_WINDOWS = 'HKLM\\SOFTWARE\\WOW6432Node\\PlayOnlineUS\\SquareEnix\\FinalFantasyXI';
const REGISTRY_PATH_WINE = 'Software\\\\WOW6432Node\\\\PlayOnlineUS\\\\SquareEnix\\\\FinalFantasyXI';
  ...
  try {
    ...
    try {
      ...
      if (padmodeMatch) {
        ...
        log.info(chalk.cyan('[Gamepad] Found padmode000 in registry:'), config.padmode000);
      }
    } catch (err) {
      log.info(chalk.yellow('[Gamepad] padmode000 not found in registry, will use existing INI value'));
    }
    ...
    try {
      ...
      if (padsinMatch) {
        ...
        log.info(chalk.cyan('[Gamepad] Found padsin000 in registry:'), config.padsin000);
      }
    } catch (err) {
      log.info(chalk.yellow('[Gamepad] padsin000 not found in registry, will use existing INI value'));
    }
  } catch (error) {
...
async function readWineRegistry(winePrefix?: string): Promise<GamepadConfig> {
  ...
  const prefix = winePrefix || process.env.WINEPREFIX || path.join(os.homedir(), '.wine');
  const systemRegPath = path.join(prefix, 'system.reg');

  try {
    if (!await fs.pathExists(systemRegPath)) {
      log.info(chalk.yellow(`[Gamepad] Wine system.reg not found at: ${systemRegPath}`));
      return config;
    }

    log.info(chalk.cyan(`[Gamepad] Reading Wine registry from: ${systemRegPath}`));
    ...
    if (sectionMatch) {
      ...
      const padmodeMatch = sectionContent.match(/"padmode000"\s*=\s*"([^"]*)"/i);
      if (padmodeMatch) {
        config.padmode000 = padmodeMatch[1];
        log.info(chalk.cyan('[Gamepad] Found padmode000 in Wine registry:'), config.padmode000);
      }

      // Parse padsin000
      const padsinMatch = sectionContent.match(/"padsin000"\s*=\s*"([^"]*)"/i);
      if (padsinMatch) {
        config.padsin000 = padsinMatch[1];
        log.info(chalk.cyan('[Gamepad] Found padsin000 in Wine registry:'), config.padsin000);
      }
    } else {
      log.info(chalk.yellow('[Gamepad] FFXI section not found in Wine registry'));
    }
  } catch (error) {
 ...
 */
export async function readGamepadConfig(winePrefix?: string): Promise<GamepadConfig> {
  const platform = process.platform;

  if (platform === 'win32') {
    log.info(chalk.cyan('[Gamepad] Reading gamepad config from Windows registry'));
    ...
    log.info(chalk.yellow(`[Gamepad] Unsupported platform: ${platform}, skipping gamepad config`));
 ...
 */
export function applyGamepadConfigToIni(config: any, gamepadConfig: GamepadConfig): void {
